...
def score_listing_fit(
    ...
    if _model_matches(normalized, recommendation):
    ...
    if normalized["price"] <= budget_max:
        ...
        reasons.append(f"Price ${normalized['price']:,} is within your ${budget_max:,} budget")
    else:
    ...
    if max_mileage is not None and "mileage" in normalized:
        ...
        if normalized["mileage"] <= max_mileage:
            ...
            reasons.append(
                f"Mileage {normalized['mileage']:,} is within your {max_mileage:,} mile limit"
            )
        else:
            warnings.append(
    ...
    if normalized.get("clean_title") is False:
        score -= DIRTY_TITLE_PENALTY
        warnings.append("Listing does not have a clean title")
    ...
    return {
        ...
        "fit_label": fit_label(normalized_score),
        "reasons": reasons,
        "warnings": warnings,
    }
